exports.run = async (client, msg, args) => {
  ...
  const userOwner = msg.guild.members.get(dbBot.idOwner);
  ...
  const queue = await client.db.bots.find({approvedDate: 0}).exec();
  queue
      .slice(queue.findIndex((bot) => bot.queuePosition === dbBot.queuePosition) + 1)
      .sort((a, b) => a.queuePosition - b.queuePosition)
      .forEach(async (bot) => {
        const queueDbBot = await client.db.bots.findOne({botId: bot.id}).exec();
