...
exports.run = async (client, msg, args) => {
  ...
  const OWNER_ID = await botManager.getOwner(userBot.id);
  ...
  if (!userOwner) {
    return client.channels.get(privateLogsChannelId).send(`Se ha detectado que elusuario <@${dbBot.ownerId}> abandonó el servidor y su bot **${userBot.tag}** está en el servidor. **ESTO AMERITA UN KICK A LA APLICACIÓN**.`);
  }

  if (args[1]) {
    if (args[1].toLowerCase() === 'vote:up') {
      // Comprueba si el usuario ya había votado positivamente.
      const usersVote = await botManager.getUsersVote(userBot.id);
      if (usersVote.includes(msg.author.id)) return msg.channel.send(':x: **Ya has votado por este bot**.');

      await botManager.voteUp(userBot.id, msg.author.id);
      ...
      await botManager.voteDown(userBot.id, msg.author.id);
      ...
      await botManager.deleteVote(userBot.id, msg.author.id);
      ...
      await botManager.setPrefix(userBot.id, args[1].slice(7));
      const newPrefix = await botManager.getPrefix(userBot.id);
      ...
      botManager.setDescription(userBot.id, description);
  ...
  const isCertified = await botManager.isCertified(userBot.id);
  const description = await botManager.getDescription(userBot.id);
  const prefix = await botManager.getPrefix(userBot.id);
  const owner = await botManager.getOwner(userBot.id);
  const votesUp = await botManager.getVotesUp(userBot.id);
  const votesDown = await botManager.getVotesDown(userBot.id);
