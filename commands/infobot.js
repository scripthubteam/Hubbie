  ...
  try {
    ...
    if (args[1]) {
      if (args[1].toLowerCase() === 'vote:up') {
        ...
        if (msg.author.id !== userOwner.id && !msg.member.roles.has("606222350228127765")) {
          return msg.channel.send(':x: **No puedes modificar información de este bot** Sólo los dueños del bot y miembros del staff de Script Hub pueden modificar esto.');
        } else {
        ...
        if (msg.author.id !== userOwner.id && !msg.member.roles.has("606222350228127765")) {
          ...
          return msg.channel.send(`**Has redefinido la descripción del bot a** \`${description.trim()}\`.`);
        }
      }
    }
