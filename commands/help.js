...
exports.run = async (client, msg, args) => {
  ...
  if (client.cmds.get(args[0])) {
    const embedHelpCMD = new RichEmbed()
      ...
      .setDescription(
        "> Alias: `" + client.cmds.get(args[0]).aliases.join(", ") + "`"
      )
    ...
    if (!client.cmds.get(args[0]).public) {
      embedHelpCMD.addField(
        "Comando administrativo",
        "> Este comando sólo puede ser usado por miembros del equipo de **Script Hub**."
      );
    }
  ...
  if (args[0])
    msg.channel.send(
      "<:shAnimeShrug:607958984854601731> No se ha encontrado el comando `" +
        args[0] +
        "` en nuestro registro de comandos. **Revisa la lista de comandos nuevamente, aquí la tienes:**"
    );
